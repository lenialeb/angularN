...
getComments(productId: string | null) {
  ...
  parseDate(dateObj: any): string {
    if (dateObj && dateObj.year && dateObj.monthValue && dateObj.dayOfMonth) {
      ...
      const formattedDate = `${String(dateObj.dayOfMonth).padStart(2, '0')}-${String(dateObj.monthValue).padStart(2, '0')}-${dateObj.year}`;
  ...
  toggleShowMore() {
    ...
    this.displayedComments = this.showAll ? this.commentList : this.commentList.slice(0, 3);
  }
}
